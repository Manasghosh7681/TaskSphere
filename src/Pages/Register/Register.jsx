...
import { errorMessage, toastType, regex } from "../../Constant/Constant";
...
const Register = () => {
    ...
    const handlValidate = () => {
        if (!state.organizationName.trim()) {
            showToast(toastType.error, errorMessage.organName);
            return false;
        }

        if (!state.industryType) {
            showToast(toastType.error, errorMessage.indType);
            return false;
        }

        if (!state.country) {
            showToast(toastType.error, errorMessage.country);
            return false;
        }

        if (!state.AdminFullName.trim()) {
            showToast(toastType.error, errorMessage.admFullName);
            return false;
        }

        if (!state.AdminEmail.trim()) {
            showToast(toastType.error, errorMessage.admemail);
            return false;
        }
        ...
        if (!emailRegex.test(state.AdminEmail)) {
            showToast(toastType.warning, errorMessage.validMail);
            return false;
        }

        if (!state.AdminPhoneNumber) {
            showToast(toastType.error,errorMessage.phone);
            return false;
        }

        // Phone length check (country code + number)
        const phoneDigits = state.AdminPhoneNumber.replace(/\D/g, '');
        if (phoneDigits.length < 10) {
            showToast(toastType.warning, errorMessage.validPhone);
            return false;
        }

        if (!state.password) {
            showToast(toastType.error, errorMessage.password);
            return false;
        }

        if (state.password.length < 6) {
            showToast(toastType.warning, errorMessage.validPAssword);
            return false;
        }

        if (!state.confirmPassword) {
            showToast(toastType.error, errorMessage.conPassword);
            return false;
        }

        if (state.password !== state.confirmPassword) {
            showToast(toastType.warning, errorMessage.validConfirmpass);
            return false;
        }

        if (!state.iAgree) {
            showToast(toastType.error, errorMessage.TermCond);
            return false;
        }
        ...
        <>
            ...
            <div className="container">
                <div className='row my-4 '>
                    <div className={`col-md-6  d-md-flex ${styles.leftPanel}`}>
